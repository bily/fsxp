...
public class ConvertUnits {
    ...
    public static String calcFSXTerrainDirectory(double latRadians, 
        ...
        if (dirNumber1 < 10)
        	dirNumber1Text = "0" + Integer.toString(dirNumber1); //$NON-NLS-1$
        else
        	dirNumber1Text = Integer.toString(dirNumber1);
        
        int dirNumber2 = ((int)(((90.0 - (latDegrees)) * 8) / 180.0));
        if (dirNumber2 < 10)
        	dirNumber2Text = "0" + Integer.toString(dirNumber2); //$NON-NLS-1$
        else
        	dirNumber2Text = Integer.toString(dirNumber2);
    ...
    public static String calcFSXTerrainFile(double latRadians, 
        ...
        if (fileNumber1 < 10)
        	fileNumber1Text = "0" +Integer.toString(fileNumber1); //$NON-NLS-1$
        else
        	fileNumber1Text = Integer.toString(fileNumber1);
        
        int fileNumber2 = ((int)(((90.0 - (latDegrees)) * 64) / 180.0));
        if (fileNumber2 < 10)
        	fileNumber2Text = "0" + Integer.toString(fileNumber2); //$NON-NLS-1$
        else
        	fileNumber2Text = Integer.toString(fileNumber2);
    ...
    public static String calcFSXTileQuad(double latRadians,
		...
		double lon = Math.toDegrees(lonRadians);
