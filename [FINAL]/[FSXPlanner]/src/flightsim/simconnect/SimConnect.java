...
public class SimConnect implements SimConnectConstants {
	 ...
	 * @param protocol SimConnect protocol version (0x2 for FSX RTM original; 0x3 for SP1)
	 * @throws IOException if connection failed
	 * @since 0.5
	 */
	public SimConnect(String appName, Configuration config, int protocol) throws IOException {
		...
		String host = config.get("Address", "localhost"); //$NON-NLS-1$ //$NON-NLS-2$
		InetAddress[] addrs = InetAddress.getAllByName(host);
		String proto = config.get("Protocol", "IPv4"); //$NON-NLS-1$ //$NON-NLS-2$
		boolean preferIpv6 = proto.equalsIgnoreCase("IPv6") ? true : false; //$NON-NLS-1$
		for (InetAddress in : addrs) {
			if (preferIpv6 && (in instanceof Inet6Address)) inAddr = in;
			if (!preferIpv6 && (in instanceof Inet4Address)) inAddr = in;
		}
		
		if (inAddr == null) throw new IOException(Messages.get("SimConnect.Unknown_host")); //$NON-NLS-1$
		
		// default SimConnect port is unknown ,just take a dumb one
		int port = config.getInt("Port", 8002); //$NON-NLS-1$
		remoteAddress = new InetSocketAddress(host, port);
		
		// buffer size
		bufferSize = config.getInt("MaxReceiveSize", RECEIVE_SIZE); //$NON-NLS-1$
		
		// protocol
		this.ourProtocol = protocol;
		if (ourProtocol != 0x2 && ourProtocol != 0x3) {
		...
		if (config.getBoolean("DisableNagle", false))  //$NON-NLS-1$
	 ...
	 * @throws IOException if connection failed
	 */
	public SimConnect(String appName, String host, int port, int protocol) throws IOException {
		this(appName, buildConfiguration(host, port), protocol);
	}
	 ...
	 * @since 0.5
	 */
	public SimConnect(String appName, int configNumber, int protocol) throws IOException, ConfigurationNotFoundException {
		this(appName, ConfigurationManager.getConfiguration(configNumber), protocol);
	}
...
<WaitForCompletion>True</WaitForCompletion>
	...
	*  parameter set to <i>"MissionCompleted"</i>. </p>
	...
	public synchronized void menu(float timeSeconds, int eventId,
			String title, String prompt, String... items) throws IOException {
	 ...
	 * 
	 */
	public synchronized void requestFacilitiesList(FacilityListType type, int eventId) throws IOException {
		// ID 0x43
	 ...
	 * 
	 */
	public synchronized void requestFacilitiesList(FacilityListType type, Enum eventId) throws IOException {
	 ...
	 * @throws IOException 
	 */
	public synchronized void subscribeToFacilities(FacilityListType type, int eventId) throws IOException {
		// ID 0x41
		clean(writeBuffer);
		writeBuffer.putInt(type.ordinal());
		writeBuffer.putInt(eventId);
	 ...
	 * @throws IOException 
	 */
	public synchronized void subscribeToFacilities(FacilityListType type, Enum eventId) throws IOException {
	 ...
	 * @throws IOException 
	 */
	public synchronized void unSubscribeToFacilities(FacilityListType type) throws IOException {
		// ID 0x42
		clean(writeBuffer);
		writeBuffer.putInt(type.ordinal());
		sendPacket(0x42);
